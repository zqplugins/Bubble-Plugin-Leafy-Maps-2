function(instance, properties, context) {
	...
	class LeafyHereRouterMachine {
		...
		addRoute(latlng, options, params, info, showRoute, showBase = true) {
			console.log("addRoute called with newShape:", latlng);
			console.log("addRoute options:", options);
			console.log("addRoute params:", params);
			console.log("addRoute routeSummary:", info);
			...
			if (routeIndex === -1) {
				...
				console.log("Map center:", instance.data.map.getCenter());
                console.log("Map zoom:", instance.data.map.getZoom());
				...
				params.forEach((element, index) => {
					...
					if (index === 0 || index === params.length - 1) {
						marker.bindTooltip(index === 0 ? 'Origin' : 'Destination', { permanent: true, className: TOOLTIP_CLASS, offset: [0, 0] });
					}
					marker.addTo(instance.data.map);
		...
		updateRoute(latlng, options = false) {
			...
			if (routeIndex !== -1 && !options) {
				...
				fetch(`https://revgeocode.search.hereapi.com/v1/revgeocode?${params}`)
					.then(response => response.json())
					.then(data => {
						console.log("Reverse geocode response:", data);  // Add this line for debugging
						...
						console.log("Calling getThisRoute with params:", sendTheseParams, "lastRouteInfo:", lastRouteInfo, "routeElement:", routeElement);
		...
		publishThisRoute(route, triggerName) {
			console.log("Publishing route:", route);
			...
			instance.publishState('here_route_traffic_time', route.actual.duration - route.actual.baseDuration);
				...
				this.retrieveAddressFromCoordinates(destination, (destAddress) => {
					...
					const waypointPromises = latList.slice(1, -1).map((lat, index) => {
						return new Promise((resolve) => {
							this.retrieveAddressFromCoordinates({ lat, lng: lngList[index + 1] }, resolve);
						});
					});

					Promise.all(waypointPromises).then((waypointAddresses) => {
						instance.publishState('here_route_marker_addresses', waypointAddresses);
		...
		addWfRouteMarker(options) {
			console.log("addWfRouteMarker options:", options);
			...
			if (options.use_address_instead && options.address) {
				...
				this.getHereGeocode({ q: options.address }, (res) => {
					...
					if (coordinates) {
						this.updateRoute({ lat: coordinates.latitude, lng: coordinates.longitude });
					} else {
				...
				console.error("Invalid input: neither valid coordinates nor valid address provided");
				instance.triggerEvent('no_route_was_found');
			}
		}
		...
		loadRoutesFromTable(options = {}) {
			...
			if (!source || typeof source.get !== 'function' || typeof source.length !== 'function') {
			...
			source.get(0, source.length()).forEach((element) => {
				...
				if (options.latitude_field && options.longitude_field) {
					...
					if (latsrc && lngsrc && typeof latsrc.get === 'function' && typeof lngsrc.get === 'function') {
				...
				if (options.route_restriction_field) {
					...
					if (restrict && typeof restrict.get === 'function') {
						restrict.get(0, restrict.length()).forEach((el) => {
							...
							if (key && value) {
								const t = key.replace(/( [a-z])/g, (group, match) => match.toUpperCase().trim());
				...
				// ROUTE CONFIGURATION
				const routeOpt = {
					name: options.name_field ? element.get(options.name_field) : `Route_${Date.now()}`,
					color: options.color_field ? element.get(options.color_field) : '#000000',
					baseColor: options.base_color_field ? element.get(options.base_color_field) : '#808080',
					deltaRoute: options.distance_limit_field ? element.get(options.distance_limit_field) : Infinity,
					restrictions: Object.keys(routeRestrictions).length > 0 ? routeRestrictions : undefined
				};

				if (parameters.length >= 2) {
					getThisRoute(parameters, routeOpt, {}, false, options.show_all_routes || false);
				} else {
	...
	const getThisRoute = (params, options, info, update = false, showRoute = true, showBase = true) => {
		console.log("getThisRoute called with params:", params);
		console.log("getThisRoute called with options:", options);
		console.log("getThisRoute info:", info);
		console.log("Update:", update, "ShowRoute:", showRoute, "ShowBase:", showBase);
		...
		// Add destination
		urlParams.append('destination', `${params[params.length - 1].latitude},${params[params.length - 1].longitude}`);
			...
			.then(res => {
				...
				if (res.routes && res.routes.length > 0) {
					...
					route.sections.forEach(section => {
						const sectionPolyline = decodeFlexiblePolyline(section.polyline).polyline;
					...
					console.log("Combined polyline:", combinedPolyline);
					console.log("Combined summary:", combinedSummary);

					if (!update) {
						instance.data.routingMachine.addRoute(combinedPolyline, options, params, combinedSummary, showRoute);
					} else {
						const diff = combinedSummary.length - info.base.length;
						console.log("Route difference:", diff);
						console.log("Route delta:", info.deltaRoute);
						if (info.deltaRoute) {
							if (diff < info.deltaRoute) {
								instance.data.routingMachine.addRoute(combinedPolyline, options, params, combinedSummary, showRoute, showBase);
							} else {
								console.log('Route limit exceeded')
...
// Helper function to decode the polyline
	...
	instance.data.actions = {
		...
		getHereRoute: (options) => {
			console.log("getHereRoute called with options:", options);
			const routeOpt = {
			...
			console.log("Initial routeOpt:", routeOpt);

			if (options.axle_count) routeOpt.vehicle.axleCount = options.axle_count;
			if (options.weight_per_axle) routeOpt.vehicle.weightPerAxle = options.weight_per_axle;
			if (options.trailer_forbidden) routeOpt.vehicle.trailerForbidden = options.trailer_forbidden;

			if (options.shipped_hazardous_goods) {
				routeOpt.vehicle.shippedHazardousGoods = options.shipped_hazardous_goods.replace(/ ([a-z])/g, '$1'.toUpperCase());
			}

			if (options.tunnel_category) routeOpt.vehicle.tunnelCategory = options.tunnel_category;

			// Update truck_type to vehicle.type
			if (options.truck_type) {
				const truckType = options.truck_type.replace(/ ([a-z])/g, '$1'.toUpperCase());
				routeOpt.vehicle.type = truckType === 'StraightTruck' ? 'StraightTruck' : 'Tractor';
			}
			...
			console.log("Final routeOpt:", routeOpt);

			if (!options.use_coordinates) {
				console.log("Using addresses for routing");
				...
				console.log("Using coordinates for routing");
				const paramsList = []
				...
				console.log("Constructed paramsList:", paramsList);
			...
			fetch(url)
				...
				.then(data => {
					...
					if (data.items && data.items.length > 0) {
						...
						const mapToState = suggestionsArray.map(item => item.address.label.replace(/,/g, ' -'));
